...
try:
    from essential.utils.ctc_alignment import ctc_forced_align
...
class SenseVoiceSmall(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Rich token slots are [language, event, emotion, textnorm].
        self.emo_rich_token_index = 2
    ...
    def _extract_emotion_distribution(self, ctc_logits: torch.Tensor, sample_idx: int, frame_idx: int):
        if frame_idx >= ctc_logits.size(1):
            return None, None, []

        emotion_labels = list(self.emo_label_to_id.keys())
        emotion_ids = torch.tensor(
            [self.emo_label_to_id[label] for label in emotion_labels],
            device=ctc_logits.device,
            dtype=torch.long,
        )

        # ctc_logits is log-softmax output, so exp(logp) is the true probability.
        raw_probs = torch.exp(ctc_logits[sample_idx, frame_idx, emotion_ids])
        norm_probs = raw_probs / raw_probs.sum().clamp_min(1e-12)
        ...
        for idx, label in enumerate(emotion_labels):
            emotion_candidates.append(
                {
                    "label": label,
                    "confidence": float(norm_probs[idx].item()),
                    "probability": float(raw_probs[idx].item()),
                }
            )
        emotion_candidates.sort(key=lambda x: x["confidence"], reverse=True)

        top_emotion = emotion_candidates[0]["label"]
        top_confidence = emotion_candidates[0]["confidence"]
        return top_emotion, top_confidence, emotion_candidates
    ...
    def inference(
        ...
        **kwargs,
        ...
        for i in range(b):
            ...
            top_emotion, top_emotion_conf, emotion_candidates = self._extract_emotion_distribution(
                ctc_logits,
                i,
                self.emo_rich_token_index,
            )
            ...
            if output_timestamp:
                ...
                result_i = {
                    ...
                    "timestamp": timestamp,
                    "emotion": top_emotion,
                    "emotion_confidence": top_emotion_conf,
    ...
    def export(self, **kwargs):
        try:
            from essential.export_meta import export_rebuild_model
        except ModuleNotFoundError:
