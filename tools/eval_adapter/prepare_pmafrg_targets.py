...
def _match_paths(root: Path, subset: str, data_dir_name: str, suffix: str) -> Dict[str, Path]:
...
def _build_subset_arrays(
    ...
    for key in common_keys:
        ...
        if emotion.ndim != 2:
            raise ValueError(f'Emotion file must be 2D: {emotion_files[key]} -> {emotion.shape}')
        if fv.ndim != 2:
            raise ValueError(f'3D_FV file must be 2D: {fv_files[key]} -> {fv.shape}')
        if emotion.shape[1] != 25:
            raise ValueError(f'Emotion target must be 25D: {emotion_files[key]} -> {emotion.shape}')
        if fv.shape[1] != 58:
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Prepare PMAFRG targets from official dataset folders.')
    parser.add_argument('--dataset-root', required=True, type=str, help='Root containing train/val subsets.')
    parser.add_argument('--subset', default='train', choices=['train', 'val'], help='Subset to export.')
    ...
    with open(out_dir / f'{args.subset}_target_summary.json', 'w', encoding='utf-8') as f:
