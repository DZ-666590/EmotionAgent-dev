...
def _load_feature(path: Path) -> np.ndarray:
    ...
    if path.suffix == ".npz":
        ...
        try:
            if len(data.files) != 1:
                raise ValueError(
                    f"{path} contains multiple arrays; please convert to single-array npz/npy first"
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="One-shot training for PMAFRG baseline adapters."
    )
    ...
    if args.emotion_features and args.emotion_targets:
        emotion_features = pmafrg_adapter.ensure_4d_features(
            _load_feature(Path(args.emotion_features))
        )
        emotion_targets = pmafrg_adapter.ensure_3d_targets(
            _load_feature(Path(args.emotion_targets)), 25
        )
        emotion_model = pmafrg_adapter.fit_ridge(
            emotion_features, emotion_targets, args.ridge_lambda
        )
        pmafrg_adapter.save_model(
    ...
    if args.face_features and args.face_targets:
        face_features = pmafrg_adapter.ensure_4d_features(
            _load_feature(Path(args.face_features))
        )
        face_targets = pmafrg_adapter.ensure_3d_targets(
            _load_feature(Path(args.face_targets)), 58
        )
        face_model = pmafrg_adapter.fit_ridge(
            face_features, face_targets, args.ridge_lambda
        )
        pmafrg_adapter.save_model(
